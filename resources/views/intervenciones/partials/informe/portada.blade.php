...
<div class="portada text-center">
    ...
    {{-- Título --}}
    <h1 style="margin-top: 20px;">Informe Mensual de Gestión</h1>
    ...
    <div style="margin-top: 40px; text-align: left; width: 80%; margin-left: auto; margin-right: auto;">
        ...
        <p><strong>Estado del informe:</strong> {{ $estado_legible }}</p>
    </div>

    {{-- Gestor --}}
    <div style="margin-top: 30px; text-align: left; width: 80%; margin-left: auto; margin-right: auto;">
        <h4>Gestor / Asesor</h4>
        <p><strong>Nombre:</strong> {{ $asesor->name ?? '' }} {{ $asesor->lastname ?? '' }}</p>
        <p><strong>Cédula:</strong> {{ $asesor->identificacion ?? 'N/A' }}</p>
        <p><strong>Correo:</strong> {{ $asesor->email ?? 'N/A' }}</p>
        <p><strong>Correo institucional:</strong> {{ $asesor->email_institucional ?? 'N/A' }}</p>
        <p><strong>Rol:</strong> {{ $asesor->rol->nombre ?? 'Asesor' }}</p>
    </div>
    ...
    <div style="margin-top: 30px; text-align: left; width: 80%; margin-left: auto; margin-right: auto;">
        ...
        <p><strong>Intervenciones realizadas:</strong> {{ $total_intervenciones }}</p>
        <p><strong>Unidades impactadas:</strong> {{ $total_unidades }}</p>
    </div>
